...
class CoreDumpGUI:
    def __init__(self, root):
        ...
        self.root.configure(bg="#0a0f1d")  # Fondo Cyberpunk Oscuro/Azulado

        # --- Paleta de Colores ---
        self.bg_dark = "#0a0f1d"
        self.bg_panel = "#111936"
        self.text_neon = "#00ffcc"  # Cian Neón
        self.text_alert = "#ff3366"  # Neón Alerta
        self.text_gold = "#ffcc00"  # Oro/Bits
        ...
        # --- Panel de Estado del Sistema (Dashboard Visual) ---
        ...
        # Indicadores Gráficos (Barras de Vida y Energía)
        ...
        # --- Panel de Controles (Botonera Sci-Fi) ---
        ...
        self.log_consola("▶️ [READY] Sistema en línea. Destruye el malware antes de que colapse el núcleo.")

    def crear_botones_estilizados(self):
        # Lista de acciones con configuraciones de colores personalizados para romper la monotonía
        acciones = [
            ...
            ("⚔️ PURGAR ENEMIGO (JEFE)", self.ejecutar_jefe, "#ff3366", "#ffffff"),
        ]
        ...
        for i, (texto, comando, color_fg, color_bg_act) in enumerate(acciones):
            ...
            # Organizar en una cuadrícula limpia
    ...
    def dibujar_barra(self, canvas, valor_actual, valor_maximo, color_barra):
        """Dibuja dinámicamente rectángulos que actúan como barras de progreso RPG."""
        canvas.delete("all")
        ancho_max = 200
        # Validar límites
    ...
    def flash_pantalla(self, color_flash):
        """Efecto visual de parpadeo (Glitch) cuando ocurre un evento importante."""
    ...
    def actualizar_pantalla(self):
        ...
        # Actualizar textos de estado básicos
        self.lbl_info_player.config(
            text=f"Usuario: {jugador.nombre}  |  🪙 Bits: {jugador.bits}  |  🎒 Inventario: {len(jugador.morral)} ítems")
        self.lbl_boss.config(text=jefe.obtener_reporte())

        # Renderizar barras gráficas dinámicas
        color_vida = "#00ff66" if jugador.integridad > 40 else "#ff3336"  # Cambia a rojo si estás muriendo
        ...
        # Estado de los sectores del juego
        if self.game.nivel_actual < len(self.game.sectores):
            sec = self.game.sectores[self.game.nivel_actual]
            self.lbl_sector.config(
                text=f"🎯 SECTOR COMPROMETIDO: {sec.nombre} (Dificultad de cifrado: {sec.dificultad})")
        else:
            self.lbl_sector.config(text="🔥 ¡TODOS LOS SECTORES REPARADOS! CORTAFUEGOS AL 100%. AISLANDO VIRUS...")

        # Fin de la partida (Derrota)
        if not jugador.esta_vivo():
            self.flash_pantalla("#550000")
            messagebox.showerror("⚠️ CRITICAL COLLAPSE ⚠️",
                                 "El sistema operativo sufrió un Kernel Panic masivo.\n¡PERDISTE EL JUEGO!")
            self.root.quit()

    # --- Ejecutores de comandos con efectos visuales adicionales ---
    def ejecutar_explorar(self):
        resultado = self.game.explorar()
        if "⚠️" in resultado:
            self.flash_pantalla("#441111")  # Flash rojo si hay error de sistema
        else:
            self.flash_pantalla("#113322")  # Flash verde si encuentra bits
    ...
    def ejecutar_jefe(self):
        ...
        self.flash_pantalla("#ff3366")  # Flash magenta de batalla armada
        self.log_consola(resultado)
        self.actualizar_pantalla()
